...
def test_e2e_delete_data_collection_filter(data_collection_filter_single):
  BASE_URL = environ.get('API_BASE_URL')
  
  req = urllib.request.Request(f"{BASE_URL}/service_engine/{data_collection_filter_single['pack_name']}/filter?filter_name={data_collection_filter_single['name']}", method='DELETE')
...
@pytest.mark.skipif(environ.get('API_BASE_URL') == None, reason="e2e not enabled")
def test_e2e_delete_data_collection_filter_resubmit(data_collection_filter_single):
  BASE_URL = environ.get('API_BASE_URL')
...
def test_e2e_delete_data_collection_filter_not_exist():
  ...
  req = urllib.request.Request(f"{BASE_URL}/service_engine/{pack_name}/filter?filter_name={dc_name}", method='DELETE')
  response = urllib.request.urlopen(req)
