...
def test_e2e_patch_document_by_id(document_single):
  ...
  req = urllib.request.Request(f"{BASE_URL}/service_engine/{document_single['pack_name']}/data_collection/{document_single['data_collection_name']}/id/{document_single['doc_id']}", data=data, method='PATCH')
...
def test_e2e_patch_document_by_id_doc_not_exist(document_single):
  ...
  data= bytes(dumps(updated_body).encode("utf-8"))
  req = urllib.request.Request(f"{BASE_URL}/service_engine/{document_single['pack_name']}/data_collection/{document_single['data_collection_name']}/id/{doc_id}", data=data, method='PATCH')
  req.add_header("Content-Type", "application/json")
